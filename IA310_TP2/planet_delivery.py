...
class SpaceRoadNetwork(Agent):
    def __init__(self, planets: List, unique_id: int, model: Model,road_branching_factor):
    ...
    def step(self):
        for e in self.current_graph.edges:
            if np.random.binomial(1,PROBA_ISSUE_ROAD) == 1:
                self.speed_modificator[e] = np.random.choice([s for s in [0 , 0.5 ,1] if s != self.speed_modificator[e]])
...
class PlanetManager(CommunicatingAgent):
    ...
    def step(self):
        ...
        for m in messages :
            body = m.body.split('|') 
            item  = Item.from_json(json.loads(body[0]))
            if item in self.waiting_for_proposal :
                self.proposals[item].append([m.sender, float(body[1])])
                messages.remove(m)
        for i in [item for item in self.waiting_for_proposal if
            ...
            if len(self.proposals[i])>0:
                best_proposal_idx  = np.argmax(proposal[1] for proposal in self.proposals[i])
                for idx,proposal in enumerate(self.proposals[i]):
                    r = spade.message.Message(
                            to=str(proposal[0]),
                            sender=str(self.communicator.jid),
                                          thread='CNP-' + str(i),
                                          metadata={"performative": "accept_proposal" 
                                                    if best_proposal_idx==idx
...
class Ship(CommunicatingAgent):
    ...
    def step(self):
        ...
        if self.item is None:
            best_utility = -1
            sender_jid = None
            best_item_serial = None
            for m in messages:
                if m.metadata["performative"]=="call_for_proposal" and (self.x, self.y) == [(plan.x, plan.y) for plan in self.planets if plan.communicator.jid == m.sender][0]:
                    item_serial, x, y = m.body.split('|')
                    item_= Item.from_json(json.loads(item_serial))
                    utility = self.utility(item_)
                    if utility> best_utility:
                        best_utility = utility
                        self.item = item_
                        self.potential_destination = [p for p in self.planets if (p.x== float(x) and p.y == float(y) )][0]
                        sender_jid = m.sender
                        best_item_serial = item_serial
            if self.item is not None:
                self.send(spade.message.Message(
                            to = str(sender_jid),
                            sender = str(self.communicator.jid),
                            body = best_item_serial + '|' + str(self.utility(self.item)),
            ...
            for m in messages:
                if (m.metadata['performative']=='accept_proposal') : 
                    print(self.communicator.name," accepted")
                    self.destination = self.potential_destination
                    self.potential_destination = None
                    break
                elif (m.metadata['performative']=='reject_proposal' ):
                    self.potential_destination = None
                    self.item = None
                    self.destination = None
                    break
    ...
    server = ModularServer(PlanetDelivery,
                           ...
                           {"n_planets": ModularVisualization.UserSettableParameter('slider',
                            ...
                            "road_branching_factor": ModularVisualization.UserSettableParameter('slider',
                                                                                  "ROAD_BRANCHING_FACTOR",
                                                                                  ROAD_BRANCHING_FACTOR,0   , 1, 0.1),
