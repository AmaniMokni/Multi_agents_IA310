...
class BidBehaviour(Behaviour):
    ...
    def execute(self, message):
        ...
        if message.performative == ACLMessage.CFP:
            bien = Good.from_string(message.content)
            offre  =str(self.bid(bien))
            reply = ACLMessage(ACLMessage.PROPOSE)
            reply.add_receiver(message.sender)
            reply.set_content(offre)
            self.agent.send(reply)
        elif message.performative == ACLMessage.ACCEPT_PROPOSAL:
            valeur_str,bien_str = message.content.split(",")
            valeur = float(valeur_str)
            bien = Good.from_string(bien_str)
            self.agent.remaining_budget -= valeur          
            bien_utility = utility(bien,self.agent)
            self.agent.overall_utility += bien_utility-valeur
...
class AuctioneerAgent(Agent):
    ...
    def initialize_send(self):
        ...
        auctioneer_behaviour = VicreyBehaviour(self)
...
class TargetBidBehaviour(BidBehaviour):
    ...
    def bid(self, good):
        return max(min(utility(good, self.agent)-0.1, self.agent.remaining_budget-0.1),0)
